...
Golang plugins must be built using the same Go libraries that Tyk is built with to avoid compatability issues.
We provide a [Docker image](https://hub.docker.com/r/tykio/tyk-plugin-compiler)
which you can use to build plugins with in order for it to be compatibility with your version of Tyk. These Docker images can be found at https://hub.docker.com/r/tykio/tyk-plugin-compiler.

Please note that the version of the  `tyk-plugin-compiler`  used must match the version of
...
This command will run the plugin compiler and compile the 3 example plugins:

- example.so
- example2.so
- example3.so

The plugins can be found in `tyk/middleware` after it successfully builds. This `.so` file can be loaded
...
This will create production-ready plugin bundles that can be found in the `tyk/bundle` directory.
...
To run the Tyk bundler tool and generate production plugin bundles, run in a terminal:
```shell
$ make bundle
```
